...
class PranaAdmin {
		...
		function settings_default() {
			...
			$prana_reset_control = false;
			$prana_options = prana_get_settings();
			
			/** Prana Reset Logic */
			if ( !is_array( $prana_options ) ) {			
				$prana_reset_control = true;			
			} 						
			elseif ( $prana_options['prana_reset_control'] == 1 ) {			
				$prana_reset_control = true;			
			}			
			
			/** Let Reset Prana */
			if( $prana_reset_control == true ) {				
				$default = $this->get_settings_default();				
				update_option( 'prana_options' , $default );			
			}
		...
		function prana_field_nav_style_fn() {
			...
			$prana_options = get_option( 'prana_options' );
